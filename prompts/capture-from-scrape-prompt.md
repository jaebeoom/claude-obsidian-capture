Runtime context의 KST date, Capture file path, Existing capture session ids, Scraped Claude.ai conversations를 기준으로 다음을 수행한다.

1. 브라우저 도구를 사용하지 않는다. 제공된 Scraped Claude.ai conversations 안의 내용만 평가한다.
2. Existing capture session ids에 이미 있는 세션은 후보에서 제외한다. 단, 최종 중복 검사는 로컬 스크립트가 다시 수행한다.
3. 각 대화를 평가한다.
...
5. source tag는 대화의 원천에 맞춘다.
   ...
   - 웹 리서치 대화이면 `#from/web`
6. PDF/책/웹 첨부자료 원문 전문은 Capture 후보에 복사하지 않는다. 그 자료를 놓고 Claude와 나눈 대화 원문만 저장한다.
7. Capture-worthy 후보가 없으면 정확히 `NO_CAPTURE_CANDIDATES`만 출력한다.
...
<!-- source: claude.ai dedicated Brave scrape -->
